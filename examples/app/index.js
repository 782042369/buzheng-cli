            ...
            () =>
                new Promise(async (resolve, reject) => {
                    try {
                        ...
                        const bufferItem = Buffer.from(pathInfo)
                        const files = await imagemin.buffer(bufferItem, {
                            plugins: [
                                imageminMozjpeg(),
                                imageminMozjpeg({
                                    progressive: true,
                        ...
                        // 压缩数据小于
                        if (Buffer.compare(files, bufferItem) === -1) {
                        ...
                        Log(Warning(`图片压缩失败, path info: ${item.path}; error info :${error.toString()}`))
                    }
                    resolve()
                })
        )
...
function squash() {
    ...
    Promise.all(squashArr).finally(() => {
        if (squashList.length === 0) {
